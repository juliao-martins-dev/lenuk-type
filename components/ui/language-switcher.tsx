    ...
    <div ref={containerRef} className={cn("relative", className)}>
      ...
      {open && (
        <div
          role="listbox"
          aria-label={t("langSwitcherLabel")}
          className="absolute right-0 top-full z-50 mt-1.5 min-w-[160px] overflow-hidden rounded-xl border border-border/70 bg-background/95 shadow-lg shadow-black/10 backdrop-blur"
        >
          {UI_LANGUAGES.map((lang) => (
            <button
              ...
              className={cn(
                "flex w-full items-center gap-2.5 px-3 py-2 text-sm transition-colors hover:bg-primary/10 hover:text-primary",
                lang.code === currentLang && "bg-primary/8 font-semibold text-primary"
              )}
            >
